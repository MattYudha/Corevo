        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center">
            <div class="d-flex align-items-center order-2 order-md-1 mt-3 mt-md-0">
                ...
                <div>
                    ...
                    <p class="text-subtitle text-muted mb-0 mt-1">Create a new letter draft.</p>
                </div>
            </div>
        ...
        <div class="container-fluid">
            <div class="card shadow-sm">
                <div class="card-body pt-4">
                    ...
                    <form action="{{ route('letters.store') }}" method="POST" id="form-letter">
                        @csrf
                        <div class="row">
                                ...
                                <select class="form-select" id="letter_template_id" name="letter_template_id">
                                    ...
                                    @foreach ($templates as $template)
                                        <option
                                            value="{{ $template->id }}"
                                            {{
                                                old('letter_template_id') == $template->id
                                                    ? 'selected'
                                                    : ''
                                            }}
                                            >{{ $template->name }}
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select" id="letter_type" name="letter_type" required>
                                    <option value="official" {{ old('letter_type') == 'official' ? 'selected' : '' }}
                                        >Official Letter
                                    </option>
                                    <option value="memo" {{ old('letter_type') == 'memo' ? 'selected' : '' }}
                                        >Memorandum
                                    </option>
                                    <option value="notice" {{ old('letter_type') == 'notice' ? 'selected' : '' }}
                                        >Notice
                                    </option>
                                </select>
                            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#form-letter').on('submit', function (e) {
                let konfirmasi = confirm('Are you sure the letter data entered is correct and ready to be saved?');
                if (!konfirmasi) {
                    e.preventDefault(); // cancel submit if cancel is clicked
                }
            });
                            ...
                            if (data.tags && data.tags.length > 0) {
                                ...
                                data.tags.forEach(function (tag) {
                                    ...
                                    if (tag.dropdown_type === 'auto_fill') {
                                        // set input to readonly so it cannot be changed arbitrarily
                                    ...
                                    // smart grid: if long_text use col-12, otherwise col-md-6
                                    let colClass = tag.input_type === 'long_text' ? 'col-12' : 'col-md-6';

                                    let inputElement = '';

                                    // render input with additional isreadonly & defaultvalue auto fill
